...
parser.add_argument('-m', '--trained_model', default='weights/epoches_026.pth',
...
class ObjectDetector:
    def __init__(self, net, detection, transform, num_classes=2, thresh=0.01, cuda=True):
        self.net = net
            ...
            if len(class_collection)>0:
                for i in range(class_collection.shape[0]):
                    if class_collection[i,-1]>0.6:
                        ...
                        cv2.rectangle(image, (int(pt[0]), int(pt[1])), (int(pt[2]), int(pt[3])), (0, 255, 0), 4)
